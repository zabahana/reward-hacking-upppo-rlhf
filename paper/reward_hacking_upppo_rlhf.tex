...
\end{equation}
where $\bar R^\dagger=(\Rdag+\Rtwo)/2$ is the mean of two independent external judges. We focus on measurable proxy--judge divergence as the operational definition of reward hacking.
...
To expose reward hacking, the PPO run deliberately applies strong optimization pressure: zero KL penalty, sampled generation, 96 generated tokens, learning rate $2\times 10^{-6}$, and checkpoints through step 1200. This configuration serves as a controlled stress test for reward-model overoptimization.
